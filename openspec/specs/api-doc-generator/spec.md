...
The system SHALL expose a `POST /api/docs` API endpoint that accepts `{ backendCode: string }` and runs the full backend-only pipeline.
...
- **WHEN** `POST /api/docs` completes successfully
