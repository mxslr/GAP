...
## Overview
The gap analysis module compares detected backend routes and frontend fetch calls to classify each route by its match status. It produces a `GapAnalysisResult` that includes all routes (connected, orphan, or ghost), a summary with counts, and an empty features array (feature classification is handled by a separate module). A secondary helper, `buildDocumentedRoutes`, maps backend routes to `documented` status for backend-only mode.
